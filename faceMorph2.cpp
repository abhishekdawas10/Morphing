...
//    cout<<"img1"<<img1Cropped.at<float>(0,1)<<endl;
   ...
   // cout<<"warp="<<warpMat1.rows<<" "<<warpMat1.cols<<" "<<warpMat1.at<float>(0,1);
    ...
    for(int a=0; a<warpImage1.rows; a++){
     ...
     warpMat2 = getAffineTransform(t2Cropped,tCropped); // Tranformation matrix is found for second img
       ...
       int xd = (warpMat2.at<double>(0,0))*a + (warpMat2.at<double>(0,1))*b + (warpMat2.at<double>(0,2));
       int yd = (warpMat2.at<double>(1,0))*a + (warpMat2.at<double>(1,1))*b + (warpMat2.at<double>(1,2));
    if(xd<img2Cropped.rows && xd>=0 && yd<img2Cropped.cols && yd>=0){
        warpImage2.at<uchar>(a,b) = (img2Cropped.at<uchar>(xd,yd));
       }
  ...
  //  
    ...
    Mat imgCropped = Mat(warpImage1.rows,warpImage1.cols,CV_8UC1);
    Mat imgCropped2 = Mat(warpImage1.rows,warpImage1.cols,CV_8UC1);//((1 - alpha)*warpImage1)+(alpha*warpImage2);
    for(int i=0;i<imgCropped.rows;i++){
    	for(int j=0;j<imgCropped.cols;j++){
    		imgCropped.at<uchar>(i,j) = (uchar)((1.0-alpha)*warpImage1.at<uchar>(i,j) + alpha*warpImage2.at<uchar>(i,j));
    	}
    }
    for(int i=0;i<imgCropped2.rows;i++){
      for (int j=0;j<imgCropped2.cols;j++){
            for(int k = 0; k < mask.cols; k++)
            {
                //mult[i][j] += a[i][k] * b[k][j];
                imgCropped2.at<float>(i,j)+=mask.at<float>(i,k) * imgCropped.at<float>(k,j);
            }
