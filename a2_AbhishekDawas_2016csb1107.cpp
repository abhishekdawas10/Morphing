...
//cout<<"Please enter the value of alpha between 0 to 1\n";
   ...
   // for(double alpha=0;alpha<=1;alpha++){
    ...
    Mat input=imread("rotation_inbuilt.jpg");
    image2="sample5.jpg";
    txt1="sample5.txt";
    txt2="sample5.txt";
    
   // Point2f pt(image.cols/2., image.rows/2.);    
   Mat r (3,3,CV_32F,Scalar(0));
    for(int i=0;i<3;i++){
      for(int j=0;j<3;j++){
      double d;
      cin>>d;
     ...
     Point2f pt(imread(image1).cols/2., imread(image1).rows/2.);    
    Mat u = getRotationMatrix2D(pt, 30, 1.0);
    warpAffine(input, dst, u, Size(input.cols, input.rows));
...
// imwrite( "rotation_inbuilt.jpg",dst);
   ...
   if(t==2){
  Mat rot = imread("trumph.jpg", CV_LOAD_IMAGE_GRAYSCALE);
  rotation(rot);
